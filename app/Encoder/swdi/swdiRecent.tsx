...
export default function SwdiRecent() {
 ...
 const columns: ColumnDef<SwdiRecord>[] = [
    ...
    },
    {
      header: "Level",
      cell: (r) => (
        <span className="text-[11px] text-(--color-muted) whitespace-nowrap">
    ...
    },
    {
      ...
      cell: (r) => (
        <span className="text-[12px] text-(--color-muted)" title={r.issue}>
  ...
  return (
    <RecentTable<SwdiRecord>
      ...
      rowClassName={(_, i) =>
        `hover:bg-blue-50/30 transition-colors duration-100 ${i % 2 === 0 ? "bg-(--color-surface)" : "bg-(--color-bg)"}`
      }
    />
  );
}
