...
export function MyRecordsTable() {
  ...
  const filteredDocs = useMemo(() => {
    ...
    return myDocuments.filter((item) => {
      if (
        s &&
        ![item.idNumber, item.name, item.drn, item.documentType, item.remarks].some((v) =>
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["recentCvs", "recentBus", "recentSwdi", "recentMisc", "recentLcn"],
      })
    ...
    <div className="p-5 bg-(--color-bg) min-h-screen font-sans antialiased space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[12px] text-(--color-muted) mt-0.5">
            Your encoded documents grouped by DRN
          </p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          All
          <span
            className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${!filters.docType ? "bg-white/20 text-(--color-bg)" : "bg-(--color-subtle) text-(--color-muted)"}`}
          >
            {myDocuments.length}
          </span>
        </button>
        {DOC_TYPES.map((t) => (
          ...
          >
            ...
            {t}
            <span
              className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${filters.docType === t ? "bg-white/20 text-(--color-bg)" : "bg-(--color-subtle) text-(--color-muted)"}`}
            >
      ...
      <div className="bg-(--color-surface) border border-(--color-border) rounded-xl p-4 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search
              size={14}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-(--color-placeholder) pointer-events-none"
            />
            <input
              type="text"
            ...
            {filters.search && (
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-(--color-placeholder) hover:text-(--color-muted) cursor-pointer bg-transparent border-none"
              >
          ...
          {activeFilters > 0 && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 text-[12px] font-medium text-(--color-muted) hover:text-(--color-ink) border border-(--color-border) hover:border-(--color-ink) rounded-lg transition-colors cursor-pointer bg-transparent shrink-0"
            >
              <X size={12} /> Clear
            </button>
          )}
        ...
        {showAdvanced && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 border-t border-(--color-border)">
            <div>
              <label htmlFor="my-records-date-from" className={labelCls}>
                Date From
              </label>
              <input
                id="my-records-date-from"
                type="date"
            ...
            <div>
              <label htmlFor="my-records-date-to" className={labelCls}>
                Date To
              </label>
              <input
                id="my-records-date-to"
                type="date"
      ...
      <div className="flex items-center justify-between">
        <p className="text-[12px] text-(--color-muted)">
          {bundles.length === 0 ? (
            ...
            <>
              <span className="font-semibold text-(--color-ink)">{bundles.length}</span> DRN bundle
              {bundles.length !== 1 ? "s" : ""} ·{" "}
              <span className="font-semibold text-(--color-ink)">{filteredDocs.length}</span>{" "}
      ...
      {isLoading ? (
        ...
        <div className="bg-(--color-surface) border border-(--color-border) rounded-xl py-20 flex flex-col items-center gap-2">
          ...
          {activeFilters > 0 && (
            <button
              onClick={clearFilters}
              className="mt-1 text-[12px] font-medium text-blue-600 hover:underline cursor-pointer bg-transparent border-none"
            >
              Clear all filters
            </button>
          )}
        ...
        <div className="space-y-2">
          {paginatedBundles.map((bundle) => {
            ...
            return (
              <div
                key={bundle.drn}
                className="bg-(--color-surface) border border-(--color-border) rounded-xl overflow-hidden transition-shadow hover:shadow-sm"
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`font-mono text-[15px] font-bold leading-tight ${isNoDrn ? "text-(--color-placeholder) italic" : "text-(--color-ink)"}`}
                    >
                      {isNoDrn ? "No DRN" : bundle.drn}
                    </p>
                    <p className="text-[13px] text-(--color-muted) mt-0.5 truncate">
                      {bundle.name}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="text-right hidden md:block">
                      <p className="text-[13px] font-medium text-(--color-ink) tabular-nums">
                        {bundle.latestDate.toLocaleDateString("en-US", {
                          month: "short",
                          day: "numeric",
                          year: "numeric",
                        })}
                    ...
                    <div
                      className={`w-8 h-8 rounded-lg border border-(--color-border) flex items-center justify-center shrink-0 transition-colors ${isExpanded ? "bg-(--color-ink) border-(--color-ink)" : "bg-(--color-surface)"}`}
                    >
                      <ChevronDown
                        size={15}
                ...
                {isExpanded && (
                  <div className="border-t border-(--color-border) bg-(--color-bg) divide-y divide-(--color-border)">
                    {bundle.docs.map((doc) => {
                      const borderColor =
                        doc.documentType === "BUS"
                          ? "border-indigo-400"
                          : doc.documentType === "SWDI"
                            ? "border-emerald-400"
                            : doc.documentType === "PCN"
                              ? "border-rose-400"
                              : doc.documentType === "CVS"
                                ? "border-sky-400"
                                : "border-amber-400"
                      ...
                      return (
                        <div
                          key={doc.id}
                          className={`flex flex-col gap-4 px-5 py-5 border-l-4 ${borderColor} hover:bg-(--color-surface) transition-colors`}
                        >
                          {/* Top row — info fields */}
                          <div className="flex flex-wrap items-start gap-6">
                            {/* Doc type badge */}
                            <div className="flex flex-col gap-1.5">
                              <span className="text-[10px] font-semibold text-(--color-placeholder) uppercase tracking-widest">
                                Type
                              </span>
                              <span
                                className={`inline-flex items-center font-mono text-[13px] font-bold px-3 py-1.5 rounded-lg ${moduleStyle[doc.documentType] ?? "bg-(--color-subtle) text-(--color-muted)"}`}
                              >
                                {doc.documentType}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5">
                              ...
                              <span className="font-mono text-[15px] font-semibold text-(--color-ink) leading-none">
                                {doc.idNumber}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5">
                              ...
                              <span className="font-mono text-[15px] font-semibold text-(--color-ink) leading-none">
                                {doc.typeOfUpdate + " - " + UPDATE_TYPE_KEYMAP[doc.typeOfUpdate]}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5 flex-1 min-w-40">
                              ...
                              <span className="text-[14px] text-(--color-ink) leading-snug">
                                {doc.subjectOfChange || (
                                  <span className="text-(--color-placeholder) italic font-normal text-[13px]">
                                    None
                                  </span>
                                )}
                              </span>
                          ...
                          <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-(--color-border)">
                            <span className="text-[11px] text-(--color-placeholder) mr-1">
                              Actions:
                            </span>
                            <button
      ...
      {totalPages > 1 && (
        <div className="bg-(--color-surface) border border-(--color-border) rounded-xl px-5 py-3.5 flex items-center justify-between">
          <p className="text-[12px] text-(--color-muted)">
            Page <span className="font-semibold text-(--color-ink)">{currentPage}</span> of{" "}
            <span className="font-semibold text-(--color-ink)">{totalPages}</span>
            ...
            <span className="ml-2">
              ...
              {" – "}
              <span className="font-semibold text-(--color-ink)">
                {Math.min(currentPage * pageSize, bundles.length)}
              </span>
            </span>
          </p>
