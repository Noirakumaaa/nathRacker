...
export default function EmployeesTab() {
  ...
  const filterableLgus =
    filterOffice === "" ? lgus : lgus.filter((l) => l.operationsOfficeNumId === filterOffice)
  const filterableBarangays =
    filterLgu === "" ? barangays : barangays.filter((b) => b.lguId === filterLgu)
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    const matchBarangay =
      filterBarangay === "" || emp.userInfo?.assignedBarangayId === filterBarangay
  ...
  const filteredLgus =
    form.assignedOperationId === ""
      ? lgus
      : lgus.filter((l) => l.operationsOfficeNumId === form.assignedOperationId)
  const filteredBarangays =
    form.assignedLGUID === "" ? barangays : barangays.filter((b) => b.lguId === form.assignedLGUID)
  ...
  const handle = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setForm((p) => {
      ...
      if (name === "assignedOperationId") {
        next.assignedLGUID = ""
        next.assignedBarangayId = ""
      }
      if (name === "assignedLGUID") {
        next.assignedBarangayId = ""
      }
      return next
    })
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          ...
          {showFilters && (
            <div className="bg-(--color-surface) rounded-xl border border-(--color-border) p-4 space-y-3">
              ...
              <div>
                <label htmlFor="employee-filter-office" className={labelCls}>
                  Operations Office
                </label>
                <select
                  ...
                  value={filterOffice}
                  onChange={(e) => {
                    setFilterOffice(e.target.value === "" ? "" : Number(e.target.value))
                    setFilterLgu("")
                    setFilterBarangay("")
                  }}
                  className={inputCls}
              ...
              <div>
                <label htmlFor="employee-filter-lgu" className={labelCls}>
                  LGU
                </label>
                <select
                  id="employee-filter-lgu"
                  value={filterLgu}
                  onChange={(e) => {
                    setFilterLgu(e.target.value === "" ? "" : Number(e.target.value))
                    setFilterBarangay("")
                  }}
                  className={inputCls}
                >
                  <option value="">All LGUs</option>
                  {filterableLgus.map((l) => (
                    <option key={l.id} value={l.id}>
                      {l.name} — {officeName(l.operationsOfficeNumId)}
                    </option>
                  ))}
                </select>
              </div>
              <div>
                <label htmlFor="employee-filter-barangay" className={labelCls}>
                  Barangay
                </label>
                <select
                  id="employee-filter-barangay"
                  value={filterBarangay}
                  onChange={(e) =>
                    setFilterBarangay(e.target.value === "" ? "" : Number(e.target.value))
                  }
                  className={inputCls}
                >
                  <option value="">All Barangays</option>
                  {filterableBarangays.map((b) => (
                    <option key={b.id} value={b.id}>
                      {b.name} — {lguName(b.lguId)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              {!isLoading && filteredEmployees.length === 0 && (
                <div className="py-12 flex flex-col items-center gap-2 text-(--color-muted)">
                  ...
                  <p className="text-[12px]">
                    {search || hasActiveFilters ? "No results found." : "No employees yet."}
                  </p>
                </div>
              )}
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-[11px] font-bold ${
                        isSelected
                          ...
                          : "bg-(--color-subtle) text-(--color-ink)"
                      }`}
                    >
                      {initials(emp)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-[13px] font-semibold truncate ${isSelected ? "text-white" : "text-(--color-ink)"}`}
                      >
                        {emp.userInfo?.firstName} {emp.userInfo?.lastName}
                      </p>
                      <p
                        className={`text-[11px] truncate ${isSelected ? "text-white/55" : "text-(--color-muted)"}`}
                      >
                        {emp.govUsername}
                      </p>
                    </div>
                    ...
                    <ChevronRight
                      size={12}
                      className={`shrink-0 ${isSelected ? "text-white/30" : "text-(--color-placeholder)"}`}
                    />
                  </button>
                )
              })}
              ...
              >
                ...
                <div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <div>
                      <label htmlFor="employee-first-name" className={labelCls}>
                        First Name <Req />
                      </label>
                      <input
                    ...
                    <div>
                      <label htmlFor="employee-last-name" className={labelCls}>
                        Last Name <Req />
                      </label>
                      <input
                    ...
                    <div>
                      <label htmlFor="employee-middle-name" className={labelCls}>
                        Middle Name <Opt />
                      </label>
                      <input
                    ...
                    <div>
                      <label htmlFor="employee-phone" className={labelCls}>
                        Phone <Req />
                      </label>
                      <input
                        id="employee-phone"
                ...
                <div>
                  ...
                  <div className="mt-3">
                    <label htmlFor="employee-role" className={labelCls}>
                      Role <Req />
                    </label>
                    <select
                      id="employee-role"
                ...
                <div>
                  ...
                  <div className="mt-3 space-y-3">
                    <div>
                      <label htmlFor="employee-operation" className={labelCls}>
                        Operations Office <Opt />
                      </label>
                      <select
                        id="employee-operation"
                        name="assignedOperationId"
                        value={form.assignedOperationId}
                        onChange={handle}
                        className={inputCls}
                      >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label htmlFor="employee-lgu" className={labelCls}>
                          LGU <Opt />
                        </label>
                        <select
                          id="employee-lgu"
                          name="assignedLGUID"
                          value={form.assignedLGUID}
                          onChange={handle}
                          className={inputCls}
                        >
                      ...
                      <div>
                        <label htmlFor="employee-barangay" className={labelCls}>
                          Barangay <Opt />
                        </label>
                        <select
