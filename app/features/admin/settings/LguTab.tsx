...
export default function LguTab() {
  ...
  const remove = async (id: number) => {
    try {
      ...
      const message = error instanceof Error ? error.message : "Unable to remove LGU."
      show(message, "error")
    }
  }
      ...
      const parsed = rows
        .map((r) => ({
          name: String(r["name"] ?? r["Name"] ?? r["NAME"] ?? "")
            .trim()
            .toUpperCase(),
          office: String(r["office"] ?? r["Office"] ?? r["OFFICE"] ?? r["operationsOffice"] ?? "")
            .trim()
            .toUpperCase(),
        }))
        .filter((r) => r.name)
  ...
  const matchable =
    importRows?.filter((r) => offices.find((o) => o.name.toUpperCase() === r.office)).length ?? 0
        ...
        >
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                <label htmlFor="lgu-name" className={labelCls}>
                  LGU Name <Req />
                </label>
                <input
                  id="lgu-name"
              ...
              <div>
                <label htmlFor="lgu-office" className={labelCls}>
                  Operations Office <Req />
                </label>
                <select
                  id="lgu-office"
                  value={form.operationsOfficeNumId}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, operationsOfficeNumId: Number(e.target.value) }))
                  }
                  required
                  className={inputCls}
                >
              ...
              <SubmitRow
                loading={loading}
                submitLabel="Add LGU"
                onCancel={() => setForm({ name: "", operationsOfficeNumId: "" })}
              />
            </div>
          </div>
        </form>
            ...
            {importRows === null ? (
              ...
              >
                ...
                <div className="flex items-center gap-1 text-[10px] text-(--color-placeholder)">
                  ...
                  <span>+</span>
                  <span className="font-mono bg-(--color-border) px-1.5 py-0.5 rounded">
                    office
                  </span>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-3 px-3 py-2.5 bg-(--color-subtle) rounded-lg border border-(--color-border)">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-medium text-(--color-ink) truncate">
                      {fileName}
                    </p>
                    <p className="text-[10px] text-(--color-muted)">
                      ...
                      {importRows.length - matchable > 0 && (
                        <span className="text-red-500">
                          , {importRows.length - matchable} unmatched
                        </span>
                      )}
                    </p>
                  </div>
                ...
                <div className="max-h-44 overflow-y-auto space-y-1 rounded-lg border border-(--color-border) p-2">
                  {importRows.map((r, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`flex items-center gap-2 px-2.5 py-1.5 rounded-md ${matched ? "bg-emerald-50" : "bg-red-50"}`}
                      >
                        {matched ? (
                        ...
                        <span
                          className={`text-[12px] font-medium ${matched ? "text-emerald-800" : "text-red-700"}`}
                        >
                          {r.name}
                        </span>
                        <span
                          className={`text-[10px] ml-auto ${matched ? "text-emerald-500" : "text-red-400"}`}
                        >
                          {r.office || "(no office)"}
                          {!matched && " · not found"}
                        </span>
        ...
        <div className="bg-(--color-surface) rounded-xl border border-(--color-border) overflow-hidden lg:col-span-2">
          ...
          <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-80 overflow-y-auto">
            {isLoading ? (
              ...
              <p className="text-[12px] text-(--color-muted) text-center py-8 col-span-full">
                No LGUs added yet.
              </p>
            ) : (
              lgus.map((l) => (
                <ListItem
                  key={l.id}
                  label={l.name}
                  sub={officeName(l.operationsOfficeNumId)}
