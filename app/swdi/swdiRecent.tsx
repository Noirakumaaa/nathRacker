...
const buildColumns = (onEdit: (record: SwdiData) => void): ColumnDef<SwdiData>[] => [
  {
    header: "HH ID",
    cell: (r) => (
  ...
  {
    header: "Grantee",
    cell: (r) => <span className="text-[12px] font-medium text-[#1a1a18] whitespace-nowrap">{r.grantee}</span>,
  },
  {
    header: "SWDI Score",
    cell: (r) => <span className="font-mono text-[13px] font-semibold text-[#1a1a18]">{r.swdiScore}</span>,
  },
  {
    header: "Level",
    cell: (r) => (
...
export default function SwdiRecent() {
  ...
  const handleEdit = (record: SwdiData) => {
    const { createdAt, updatedAt, id: _, userId, remarks, swdiScore, ...rest } = record;
    dispatch(setCurrentSwdiForm({ ...rest, remarks, swdiScore, cl: rest.cl || "", drn: rest.drn || "" }));
  };

  useEffect(() => {
    if (newSwdiData) dispatch(setNewData(false));
  }, [newSwdiData]);

  return (
    <RecentTable<SwdiData>
      queryKey="recentSwdi"
