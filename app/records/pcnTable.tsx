...
import type { PcnData } from "./records";

export function PcnTable() {
    ...
    const { data: pcnData = [], isLoading, refetch } = useQuery<PcnData[]>({
        queryKey: ['pcnData', User.id],
        queryFn: async () => await get(`${import.meta.env.VITE_BACKEND_API_URL}/v1/pcn/records?id=${User.id}`),
        enabled: !!User.id
    });

    const filteredData = useMemo(() => pcnData.filter(item => {
        const searchTerm = filters.search.toLowerCase();
        const matchesSearch = filters.search ? [
            item.hhId, item.grantee, item.pcn || '', item.tr || '', item.issue || '', item.date
    ...
    const getPcnStatus = (pcn: string | null) => {
        if (!pcn || pcn.trim() === '') return { text: 'None', class: 'text-red-700 bg-red-100' };
        return { text: 'Available', class: 'text-green-700 bg-green-100' };
    };

    const getTrStatus = (tr: string | null) => {
        if (!tr || tr.trim() === '') return { text: 'None', class: 'text-red-700 bg-red-100' };
        return { text: 'Available', class: 'text-green-700 bg-green-100' };
    };
                                        ...
                                        <td className="px-4 py-4 text-sm">
                                            <div className="flex items-center gap-2">
                                                {item.tr ? (
                                                    ...
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getTrStatus(item.tr).class}`}>
                                                        {getTrStatus(item.tr).text}
                                                    </span>
                                                )}
                                            </div>
                                        </td>
